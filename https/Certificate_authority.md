...
**Setup Workflow**:
1. Run `scripts/SSL_Details.sh` to configure SSL settings (prompts for installation type, provider, email, etc.)
2. Script automatically runs `scripts/Finding_Hostname.sh` to detect your domain
3. Run `scripts/SSL_Creation.sh` to install cert-manager and apply the configuration to your cluster
